...
void main() {
  ...
  test('file can move to trash and create a database record',
          () async {
        ...
        final storedItem =
        await repository.getTrashItem(item.id);
  ...
  test(
      'expired files are permanently deleted',
          () async {
        final storage = FakeFileStorage();
        ...
        await repository.insertTrashItem(
          TrashItem(
            ...
            fileId: 'file-old',
            originalPath:
            '${storage.received.path}/old.pdf',
            trashPath: file.path,
            deletedAt: DateTime.now()
                .subtract(const Duration(days: 61)),
            permanentDeleteAt: DateTime.now()
